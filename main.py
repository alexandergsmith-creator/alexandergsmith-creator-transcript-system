...
COOKIES_FILE = "cookies.txt"  # Ensure this is in your GitHub repo!
...
def harvest_video(url, api):
    ...
    # yt-dlp config: Using Cookies and Sleep to avoid 429/Bot detection
    download_cmd = [
        "yt-dlp",
        "-x", "--audio-format", "wav",
        ...
        "--sleep-interval", "10",        # Wait 10s between videos to look human
        "--max-sleep-interval", "30",    # Randomize wait up to 30s
    ]

    # Only add cookies flag if the file actually exists
    if os.path.exists(COOKIES_FILE):
        download_cmd.extend(["--cookies", COOKIES_FILE])
    else:
        log("⚠️ WARNING: cookies.txt not found. YouTube might block this request.")
    ...
    try:
        ...
        if not os.path.exists("temp_audio.wav"):
            log("⏭️ Video skipped (already in archive or limit reached).")
            return
        ...
        # THE PURGE: Absolute Storage Safety for 1GB limit
        if os.path.exists("temp_audio.wav"): os.remove("temp_audio.wav")
        if os.path.isdir("chunks"): shutil.rmtree("chunks")
        log("🧹 Railway storage cleared.")
